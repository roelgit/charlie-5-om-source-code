...
\item We hypothesised that the runtime of the algorithm increases quadratically as the number of input vertices increases. This has been proven mostly true. As shown in the figure below, both F\&R and Eames are a good fit, with Coëfficients of Determination above 0.95. Furthermore, a two-tailed, paired-samples t-test shows that the runtime values are clearly unrelated with $p = 3.8 * 10^{-17}$. Interestingly, Eames shows $O(n^3)$-like behaviour, whereas $O(n^2)$ is expected. F\&R does behave as expected.
\begin{figure}[h]
  \centering
